...
import { NotesBlock } from './src/components/common/NotesBlock';
...
type MaterialCategory = 'plaster' | 'putty' | 'paint' | 'wallpaper' | 'tile' | 'flooring' | 'screed' | 'skirting' | 'drywall';
    ...
    return (
        <div className="calc-input-group">
            ...
            <div className="calc-input-wrapper">
                <input ref={ref} id={id} type="number" value={value} onChange={handleChange} min="0" step={props.step || "0.01"} className={error ? 'invalid' : ''} aria-invalid={!!error} aria-describedby={error ? `${id}-error` : undefined} {...props} />
...
const SkirtingCalculator: React.FC<{ perimeter: number, totalDoorWidth: number, totalExclusionWidth: number } & MaterialCalculatorProps> = ({ perimeter, totalDoorWidth, totalExclusionWidth, name, onResultChange, materials }) => {
    ...
    const result = useMemo(() => {
        ...
        const nMargin = parseInt(margin, 10) || 0;
        const nPrice = parseFloat(price.replace(',', '.')) || 0;
        if (nPerimeter === 0 || nSkirtingLength === 0) return null;
...
const DrywallCalculator: React.FC<{ wallArea: number, floorArea: number, perimeter: number } & MaterialCalculatorProps> = ({ wallArea, floorArea, perimeter, name, onResultChange, materials }) => {
    ...
    const results = useMemo(() => {
        ...
        const nMargin = parseInt(margin, 10) || 0;
        if (area === 0) return { lines: [], quantity: '', cost: 0, details: {}, showNote: false };
...
const RoomEditor: React.FC<{
    ...
    const handleAddExclusion = () => handleRoomChange(activeRoomId, 'exclusions', [...activeRoom.exclusions, { id: Date.now(), name: '', width: '', height: '', count: '1', affectsPerimeter: false, affectsWallArea: true }]);
    ...
    return (
        <>
            ...
            <NotesBlock 
                ...
                onNotesChange={(notes) => handleRoomChange(activeRoomId, 'notes', notes)}
            ...
            <ImageUpload 
                ...
                onImagesChange={(images) => handleRoomChange(activeRoomId, 'images', images)}
                roomName={activeRoom.name}
            />
...
const ResultsPage: React.FC<{
    ...
    materials: SavedMaterial[];
}> = ({ rooms, materialResults, onMaterialResultChange, materials }) => {
    
    const [activeFilters, setActiveFilters] = useState<string[]>(['Стены', 'Потолок', 'Пол', 'Прочее']);
    ...
    const totalCalculations = useMemo(() => {
        return rooms.reduce((totals, room) => {
            ...
            totals.roomCount += 1;
            return totals;
        }, { totalFloorArea: 0, totalPerimeter: 0, totalNetWallArea: 0, totalDoorWidth: 0, totalExclusionPerimeterLength: 0, totalDoorPerimeter: 0, avgHeight: 0, roomCount: 0 });
    }, [rooms]);
    
    const totalCost = useMemo(() => {
        return Object.values(materialResults).reduce((sum, result) => sum + (result?.cost || 0), 0);
    }, [materialResults]);
    ...
    const handleShareCalculations = () => {
        ...
        MATERIAL_ORDER.forEach(key => {
            ...
            if (result && result.quantity.trim()) {
    ...
    const handleExportPdf = () => {
        ...
        // Add font supporting cyrillic characters
        // Note: jsPDF has limited font support. For full compatibility, embedding a font (e.g., with VFS) is recommended.
        // This example uses a standard font that might work in some environments.
        ...
        doc.save(`smetа_${new Date().toISOString().slice(0, 10)}.pdf`);
    };
                    ...
                    {MATERIAL_ORDER.map(name => {
                        ...
                        return (
                            <div className="summary-item" key={name}>
                                <span>{name}</span>
                                <strong className="summary-item__value">{result.quantity}<br/>{result.cost.toFixed(2)} ₽</strong>
                            </div>
                        );
                    })}
                </div>
...
const CalculatorView: React.FC<{
    ...
    onOpenLibrary: () => void;
    materials: SavedMaterial[];
}> = ({ rooms, setRooms, handleInputFocus, handleThemeChange, themeIcon, onSaveEstimate, onLoadEstimate, onOpenLibrary, materials }) => {
    const [step, setStep] = useState(1);
    const [activeRoomId, setActiveRoomId] = useState<number>(rooms[0]?.id || 0);
        ...
        <>
            <header className="calc-header">
                <div className="calc-header-left">
                    {step === 2 && <button onClick={() => setStep(1)} className="back-btn"><IconChevronRight style={{transform: 'rotate(180deg)'}} /></button>}
                ...
                <div className="header-actions">
                    ...
                    <button onClick={handleThemeChange} className="header-btn" aria-label="Сменить тему">{themeIcon()}</button>
                </div>
            </header>
            <main>
                {step === 1 ? (
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Сохранить расчет">
            <div className="modal-body">
                <CalcInput id="estimateName" label="Название расчета" value={name} onChange={e => setName(e.target.value)} ref={inputRef} />
            </div>
...
const MaterialLibraryModal: React.FC<{
    ...
    const categoryFields: Record<MaterialCategory, {key: string, label: string, unit: string}[]> = {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Библиотека материалов">
            {view === 'list' ? (
                ...
                <>
                    <div className="modal-body">
                        <CalcInput id="matName" label="Название материала" value={name} onChange={e => setName(e.target.value)} />
                        <div className="calc-input-group">
                        ...
                        <div className="form-grid">
                            {categoryFields[category].map(field => (
                                <CalcInput key={field.key} id={`matParam-${field.key}`} label={field.label} unit={field.unit} value={params[field.key] || ''} onChange={e => handleParamChange(field.key, e.target.value)} />
                            ))}
                        </div>
                    </div>
                    <div className="modal-footer">
...
const App: React.FC = () => {
    ...
    const [rooms, setRooms] = useState<RoomData[]>([getDefaultRoom()]);
        ...
        try {
            ...
            if (autosavedData) {
                ...
                if (Array.isArray(restoredRooms) && restoredRooms.length > 0) {
                    setTimeout(() => {
                        safeShowConfirm('Найдена незаконченная сессия. Восстановить введенные данные?', (ok) => {
                            if (ok) {
    ...
    const handleLoadEstimate = (id: number) => {
        ...
        safeShowConfirm('Загрузка сохраненного расчета перезапишет текущие данные. Продолжить?', (ok) => {
            if (ok) {
